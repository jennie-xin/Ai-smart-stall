...
const title = ref('我的商品配置' + `（${list.value.length}）`)
</script>
...
<template>
  <view class="container">
	<!-- 头部栏 -->
	<uni-nav-bar status-bar :title="title" left-icon="left"  @clickLeft="uni.navigateBack()" />
	...
	<view class="list">
		<view class="product-card" v-for="item in list" :key="item.id">
			...
			<view class="product-info">
				...
				<view class="opt">
					<button type="primary" class="btn" @click="addToConfig(1)">进货</button>
					<button type="warn" class="btn" @click="addToConfig(1)">删除</button>
				</view>
			</view>
		</view>
	</view>
